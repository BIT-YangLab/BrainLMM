...
layer_dic = {'conv1':0,'layer1':1, 'layer2':2, 'layer3':3, 'layer4':4, 'fc':'fc', 'encoder':'encoder','visual.attnpool':'image_embedding','classifier.0':'classifier.0','classifier[0]':'classifier[0]','avgpool':'avgpool','visual':'image_embedding'}
...
def get_activation_brain(outputs, mode,weights):
    '''
    mode: how to pool activations: one of avg, max
    for fc or ViT neurons does no pooling
    '''
    if mode=='avg':
        def hook(model, input, output):
            if len(output.shape)==4: #CNN layers
                ...
                #outputs.append(output.detach())
                outputs.append(torch.einsum('ij,jk->ik',output,weights).detach())
    elif mode=='max':
        def hook(model, input, output):
            if len(output.shape)==4: #CNN layers
                outputs.append(output.amax(dim=[2,3]).detach())
...
def save_target_activations_brain(target_model, dataset, save_name, target_layers = ["layer4"], batch_size = 1000,
    ...
    for target_layer in target_layers:
        ...
        command = "target_model.{}.register_forward_hook(get_activation_brain(all_features[target_layer], pool_mode,weights))".format(target_layer)
        hooks[target_layer] = eval(command)

    with torch.no_grad():
        if "tile2vec" not in save_name:

            if "clip" in save_name:
                for images, labels in tqdm(DataLoader(dataset, batch_size, num_workers=8, pin_memory=True)):
...
def save_activations_brain(target_name, target_layers, d_probe, 
                    batch_size, device, pool_mode, base_dir, saved_acts_dir, model=None, preprocess=None,weights=None,roi=None):
    if target_name == 'custom':
        target_model, target_preprocess = data_utils.get_target_model(target_name, device, model, preprocess)
    else:
        target_model, target_preprocess = data_utils.get_target_model(target_name, device)
    ...
    target_save_name = get_save_names_brain(target_name = target_name,
                                target_layer = '{}', d_probe = d_probe,
                                pool_mode=pool_mode, base_dir = base_dir, saved_acts_dir = saved_acts_dir,roi=roi)

    save_target_activations_brain(target_model, data_t, target_save_name, target_layers,
                            batch_size, device, pool_mode,weights=weights)
    return




def create_layer_folder_brain(results_dir, base_dir, target_name, d_probe, layer, roi):

    sys.path.append(base_dir)
    save_folder_name = '{}_{}_layer{}_results_{}'.format(target_name, d_probe, layer_dic[layer], roi)
    file_path = os.path.join(base_dir, results_dir, save_folder_name)
...
def get_save_names_brain(target_name, target_layer, d_probe, pool_mode, base_dir, saved_acts_dir,roi=None):

    saved_acts_dir = os.path.join(base_dir, saved_acts_dir)
    target_save_name = "{}/{}_{}_{}{}_{}.pt".format(saved_acts_dir, d_probe, target_name, target_layer,
                                                 PM_SUFFIX[pool_mode],roi)
    
    return target_save_name


def get_target_activations_brain(target_name, images, target_layers = ["layer4"], batch_size = 1000,
                            device = "cuda", pool_mode='avg', model=None, preprocess=None,weights=None):
    """
    gets target model activations without saving 
    """
...
def get_mean_activation_brain(outputs,weights):
    def hook(model, input, output):
        output = output.to(torch.float32)
        if len(output.shape)==4: #CNN layers
            outputs.append(torch.einsum('ijkm,ja->iakm',output,weights).detach())
        elif len(output.shape)==3: #ViT
            outputs.append(output[:, 0].clone())
        elif len(output.shape)==2: #FC layers
            #outputs.append(output.detach())
...
def get_activation_brain_bias(outputs, mode,weights,bias):
    '''
    mode: how to pool activations: one of avg, max
    for fc or ViT neurons does no pooling
    '''
    if mode=='avg':
        def hook(model, input, output):
            if len(output.shape)==4: #CNN layers
                outputs.append((torch.einsum('ijkm,ja->iakm',output,weights).mean(dim=[2,3])+bias).detach())
            elif len(output.shape)==3: #ViT
                outputs.append((output[:, 0]+bias).clone())
            elif len(output.shape)==2: #FC layers
                #outputs.append(output.detach())
                outputs.append((torch.einsum('ij,jk->ik',output,weights)+bias).detach())
    elif mode=='max':
        def hook(model, input, output):
            if len(output.shape)==4: #CNN layers
                outputs.append((output.amax(dim=[2,3])+bias).detach())
            elif len(output.shape)==3: #ViT
                outputs.append((output[:, 0]+bias).clone())
            elif len(output.shape)==2: #FC layers
                outputs.append((output+bias).detach())
    return hook



def save_target_activations_brain_bias(target_model, dataset, save_name, target_layers = ["layer4"], batch_size = 1000,
                            device = "cuda", pool_mode='avg',weights=None,bias=None):
    """
    save_name: save_file path, should include {} which will be formatted by layer names
    """
    _make_save_dir(save_name)
    ...
    for target_layer in target_layers:
        ...
        command = "target_model.{}.register_forward_hook(get_activation_brain_bias(all_features[target_layer], pool_mode,weights,bias))".format(target_layer)
        hooks[target_layer] = eval(command)
...
def save_activations_brain_bias(target_name, target_layers, d_probe, 
                    batch_size, device, pool_mode, base_dir, saved_acts_dir, model=None, preprocess=None,weights=None,roi=None,bias=None):
    if target_name == 'custom':
        target_model, target_preprocess = data_utils.get_target_model(target_name, device, model, preprocess)
    else:
        target_model, target_preprocess = data_utils.get_target_model(target_name, device)
    ...
    target_save_name = get_save_names_brain_bias(target_name = target_name,
                                target_layer = '{}', d_probe = d_probe,
                                pool_mode=pool_mode, base_dir = base_dir, saved_acts_dir = saved_acts_dir,roi=roi)

    save_target_activations_brain_bias(target_model, data_t, target_save_name, target_layers,
                            batch_size, device, pool_mode,weights=weights,bias=bias)
    return




def get_target_activations_brain_bias(target_name, images, target_layers = ["layer4"], batch_size = 1000,
                            device = "cuda", pool_mode='avg', model=None, preprocess=None,weights=None,bias=None):
    """
    gets target model activations without saving 
    """
...
def get_save_names_brain_bias(target_name, target_layer, d_probe, pool_mode, base_dir, saved_acts_dir,roi=None):
