...
for row_now in range(num_chunks):  
    probe="NSD"
    target_name = 'resnet50'          #resnet50     clip_resnet50      alexnet      clip_vit-b32
    target_layer = 'layer4'           #layer4       visual.attnpool    avgpool      visual        
    roi='all'
    d_probe = 'coco'
    batch_size = 200
    gpu = 1
    ...
    if target_name=='resnet50':
        weights = np.load(f'/Your_path/subj{current_subject}/34_session/weights_convnet_resnet_SELECTIVE_ROI.npy')
    #clip_rn_50
    elif target_name=='clip_resnet50': 
        weights = np.load(f'/Your_path/subj{current_subject}/34_session/weights_clip_visual_resnet_SELECTIVE_ROI.npy')
    #clip_vit-b32
    elif target_name=='clip_vit-b32':
        weights=np.load(f'/Your_path/subj{current_subject}/34_session/weights_clip_vit_SELECTIVE_ROI.npy')
    #alexnet
    else:
        weights=np.load(f'/Your_path/subj{current_subject}/34_session/weights_convnet_alexnet_SELECTIVE_ROI.npy')
    if target_name=='alexnet' and target_layer=='avgpool':
            weights = weights.T
            pca = PCA(n_components=min(weights.shape[0], 256), svd_solver="auto")
    ...
    total_ids = weights.shape[1]-n
    
    chunk_size = total_ids // num_chunks

    start = (row_now) * chunk_size+n
    end = (row_now + 1) * chunk_size+n if row_now < num_chunks - 1 else weights.shape[1]
    ...
    processor = BlipProcessor.from_pretrained("/Your_path/BLIP-main/blip-image-captioning-base")
    model = BlipForConditionalGeneration.from_pretrained("/Your_path/blip-image-captioning-base").to(device) 
    ...
    # Step 1 -Find top activating image crops
    ...
    for list_id, orig_id in enumerate(ids_to_check):
        ...
        for label_id in range(labels_to_check):
            ...
            label_folder = os.path.join("/Your_path/BrainLMM/prove/data", label.replace(" ", "_").replace("/", "_").replace("\\", "_"))
    ...
    del all_sd_imgs, all_img_ids, top_ids, target_feats
